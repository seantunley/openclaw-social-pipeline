...
const researchRoutes: FastifyPluginCallback = (
  ...
  fastify.post("/api/social/research/:id/promote", async (request, reply) => {
    try {
      ...
      // socialRun.campaign_id is NOT NULL — refuse promotion when neither the
      // body nor the research item has a campaign attached.
      const campaignId = body.campaign_id ?? research.campaign_id;
      if (!campaignId) {
        return reply.status(400).send({
          error:
            "campaign_id is required to promote research — pass it in the body or attach the research to a campaign first.",
        });
      }

      // Create a content run from this research. Platform lives inside
      // config_snapshot (there is no top-level platform column on socialRun);
      // status/trigger must use the schema enum values.
      const runId = uuidv4();
