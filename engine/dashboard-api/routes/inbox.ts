...
export const inboxRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/', async (req, reply) => {
    ...
    if (!data) {
      // Postiz isn't configured / not reachable — return empty result with
      // an `available: false` flag instead of 502'ing the dashboard.
      return reply.send({ available: false, items: [], total: 0, page: 1 });
    }
    return data;
  });
  ...
  app.get('/post/:postId/comments', async (req, reply) => {
    ...
    if (!data) return reply.send({ available: false, comments: [] });
    return data;
  });
