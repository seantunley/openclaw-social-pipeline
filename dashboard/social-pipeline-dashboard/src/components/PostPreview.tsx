...
function MediaPlaceholder({ aspect = 'aspect-square' }: { aspect?: string }) {
  return (
    <div className={`flex w-full items-center justify-center ${aspect} bg-gradient-to-br from-zinc-700 to-zinc-900 text-zinc-500`}>
    ...
    <div className="mx-auto max-w-md overflow-hidden rounded-xl border border-zinc-300/30 bg-white text-black shadow-2xl">
      <div className="flex items-center gap-3 px-3 py-2.5">
        <div className="rounded-full bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 p-[2px]">
          <Avatar initials={brandHandle[0]?.toUpperCase() ?? 'B'} color="bg-zinc-200 text-zinc-700" />
        </div>
      ...
      <div className="px-3 py-2 text-sm leading-snug">
        ...
        {rest && (
          <p className="mt-1 whitespace-pre-wrap text-zinc-700">{rest}</p>
        )}
        <p className="mt-2 text-xs uppercase tracking-wide text-zinc-400">3 hours ago</p>
      </div>
    </div>
  );
}
...
function FacebookPreview({ content, mediaUrl, brandName }: { content: string; mediaUrl?: string | null; brandName: string }) {
  return (
    <div className="mx-auto max-w-md overflow-hidden rounded-lg border border-zinc-300/30 bg-white text-black shadow-2xl font-sans">
      ...
      <div className="flex items-center justify-between border-t border-zinc-200 px-4 py-2 text-sm text-zinc-600">
  ...
  return (
    <div className="mx-auto max-w-md overflow-hidden rounded-lg border border-zinc-300/30 bg-white text-black shadow-2xl">
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="truncate text-sm font-semibold">{brandName}</p>
          <p className="truncate text-xs text-zinc-500">Sponsored · Following</p>
          <p className="flex items-center gap-1 text-xs text-zinc-500">
            <span>3h</span>
            <span>·</span>
            <Globe className="h-3 w-3" />
      ...
      <div className="flex items-center justify-between border-t border-zinc-200 px-4 py-2 text-xs font-semibold text-zinc-600">
...
function TwitterPreview({ content, mediaUrl, brandName, brandHandle }: { content: string; mediaUrl?: string | null; brandName: string; brandHandle: string }) {
  return (
    <div className="mx-auto max-w-md overflow-hidden rounded-2xl border border-zinc-700 bg-black text-zinc-100 shadow-2xl">
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline gap-1">
            ...
            <span className="text-[14px] text-zinc-500">@{brandHandle} · 3h</span>
          </div>
      ...
      <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 capitalize">{platform}</p>
          </div>
        </div>
      </div>
      {mediaUrl ? (
